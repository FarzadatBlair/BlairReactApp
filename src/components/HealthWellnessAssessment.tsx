...
const HealthWellnessAssessment: React.FC = () => {
  ...
  const submitUserResponses = async (answers: Record<string, string[]>) => {
    try {
      ...
      //! Initialize data structure for user_medical_profile
      const userMedicalProfile: Record<
        string,
      ...
      for (const [questionId, answer] of Object.entries(answers)) {
        ...
        const { type, table, column, options } = question;

        // Handle each type of question and map answers to the correct structure
        if (table === 'user_medical_profile' && column) {
          if (type === 'NUM') {
            const numericValue = parseFloat(answer[0]);
            if (!isNaN(numericValue)) {
              userMedicalProfile[column] = numericValue;
            }
          } else if (type === 'MC') {
            userMedicalProfile[column] = answer[0];
          } else if (type === 'MS') {
            userMedicalProfile[column] = answer.join(', ');
          }
        }
      }

      // Calculate BMI if height and weight are provided
      if (userMedicalProfile.height && userMedicalProfile.weight) {
        const heightInInches = (userMedicalProfile.height as number) / 2.54; // Convert cm to inches
        const weightInLbs = userMedicalProfile.weight as number; // Weight is already in lbs
        userMedicalProfile.bmi =
      ...
      await supabase
        ...
        .upsert([{ user_id, ...userMedicalProfile }], {
          onConflict: 'user_id',
        });

      console.log('User medical profile submitted successfully!');
    } catch (err) {
